...
export async function startMatchingProcess(conversationId: string): Promise<void> {
  ...
  if (matches.length <= 2) {
    ...
    notifyHelpers(process, matches, conversationId, community_id, topic, startedAt);
  ...
  notifyHelpers(process, tier1, conversationId, community_id, topic, startedAt);
  ...
  const tier2Timeout = setTimeout(() => {
    ...
    notifyHelpers(process, tier2, conversationId, community_id, topic, startedAt);
  }, 30000);
